...
export default function StudyPage({ params }: { params: { id: string } }) {
  ...
  if (!topic || words.length === 0) return (
    <div className="min-h-screen flex flex-col items-center justify-center p-8 text-center bg-emerald-50/30">
      ...
      <Link href={`/topic/${params.id}`} className="bg-emerald-600 text-white px-10 py-5 rounded-[2rem] font-bold shadow-xl">Go back</Link>
    </div>
  );
  ...
  return (
    <main className="min-h-screen bg-emerald-50/50 p-8 md:p-24 flex flex-col items-center">
      <header className="w-full max-w-2xl flex justify-between items-center mb-12">
        <Link href={`/topic/${params.id}`} className="text-emerald-900 font-bold flex items-center gap-2 hover:opacity-70 bg-white px-6 py-3 rounded-2xl border border-emerald-100 shadow-sm transition-all">
          <ArrowLeft size={20} /> Exit
        </Link>
        <div className="flex flex-col items-end">
          <span className="text-emerald-900 font-black text-xl">{currentIndex + 1} / {words.length}</span>
      ...
      <div className="w-full max-w-2xl flex flex-col gap-8">
        ...
        <div className="text-center mt-4">
          <p className="text-gray-400 font-medium">Tip: Use Left/Right arrows or Space to navigate</p>
        </div>
      </div>
